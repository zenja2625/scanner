...
export const fromMatReturnMathes = (src: Mat, model: tf.LayersModel, ids: ID[]) => {
    ...
    rectList =
        getFirstLineY !== null
            ? rectList.filter(list => rectsListIsAfterLine(list, angle, getFirstLineY))
            : []
    ...
    const tensor = tf.tensor(numbe, [numbe.length, 30, 30, 1]).sub(offset).div(offset)
    ...
    for (let i = 0; i < rectList.length; i++) {
        ...
        const predictCode = argMax.slice(predictStartIndex, predictStartIndex + length)
